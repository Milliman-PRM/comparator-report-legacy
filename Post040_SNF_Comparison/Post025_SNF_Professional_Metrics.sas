...
proc sql;
	...
	select
		...
		,sum(prof_snf.PRM_Costs) as sum_costs_snf
		,sum(prof_snf.prm_costs / risk.riskscr_1_cost_avg) as _sum_costs_snf_riskadj
...
proc sql;
	...
	select
		...
		,sum(detail.sum_costs_snf)
			...
			as pct_SNF_costs label="Professional SNF Costs as a Percentage of Total Costs"
